...
restore() {
    ...
    if [[ -n "$BUILD_SH_BACKUP" && -f "$BUILD_SH_BACKUP" ]]; then
        cp -f "$BUILD_SH_BACKUP" tools/depot-to-apk/build.sh
        rm -f "$BUILD_SH_BACKUP"
    fi
}
...
# The final APK manifest is generated by tools/depot-to-apk/build.sh rather
# than merged from the launcher's AAR manifest. Keep the generated manifest in
# lock-step with the AAR: AchievementService must be declared there too or
# Android accepts the start request with a null ComponentName and the service
# never reaches onCreate(). Patch the packaging script inside the disposable
# build container and restore the checkout on exit.
BUILD_SH_BACKUP="$(mktemp)"
cp -f tools/depot-to-apk/build.sh "$BUILD_SH_BACKUP"
python3 - <<'PY'
from pathlib import Path

path = Path("tools/depot-to-apk/build.sh")
text = path.read_text()

fgs = '    <uses-permission android:name="android.permission.FOREGROUND_SERVICE" />\n'
if fgs not in text:
    needle = '    <uses-permission android:name="android.permission.INTERNET" />\n'
    if needle not in text:
        raise SystemExit("cannot patch generated manifest: INTERNET permission anchor missing")
    text = text.replace(needle, needle + fgs, 1)

service = '''        <service android:name="dev.silksong.launcher.AchievementService"
            android:exported="false" android:process=":launcher" />
'''
if service not in text:
    needle = '''        <service android:name="dev.silksong.launcher.MonoService"
            android:exported="false" android:process=":builder" />
'''
    if needle not in text:
        raise SystemExit("cannot patch generated manifest: MonoService anchor missing")
    text = text.replace(needle, needle + service, 1)

path.write_text(text)
PY

grep -Fq 'android.permission.FOREGROUND_SERVICE' tools/depot-to-apk/build.sh \
    || { echo "[docker] achievement manifest patch lost FOREGROUND_SERVICE permission" >&2; exit 1; }
grep -Fq 'dev.silksong.launcher.AchievementService' tools/depot-to-apk/build.sh \
    || { echo "[docker] achievement manifest patch lost AchievementService declaration" >&2; exit 1; }
say "achievement service declaration injected into final APK manifest generator"
